...
Model Hypothesis: Based on empirical knowledge, birth weight may be linearly related to pre-pregnant weight and gestational age in weeks. Here is my model:
```{r}
...
```{r residuals_vs_gaweeks, fig.cap="residuals_vs_gaweeks"}
bw_df |> 
  modelr::add_residuals(fit_hypothetical) |> 
...
```
```{r qqplot_residual, fig.cap="qqplot_residual"}
...
```
The qq plot shows that the residuals are approximately approach normal distribution.
